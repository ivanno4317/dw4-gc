...
void GXSetArray(GXAttr attr, void* base_ptr, u8 stride) {
    ...
    attrReg = attr - GX_VA_POS;

    GX_CP_LOAD_REG(
        attrReg | 0xA0,
        (u32)base_ptr & ~0xC0000000
    );

    newAttr = attrReg - 0xC;
    if ((newAttr >= 0) && (newAttr < 4)) {
        __GXData->indexBase[newAttr] = (u32)base_ptr & ~0xC0000000;
    }
    ...
    __GXData->indexStride[newAttr] = stride;
    return;
}
