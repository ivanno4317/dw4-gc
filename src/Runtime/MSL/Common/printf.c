...
static int __pformatter(void *(*WriteProc)(void *, const char *, size_t), void * WriteProcArg, const char * format_str, va_list arg) {
	...
	while (*format_ptr) {
		...
		case 'X':
			...
			else if (!(buff_ptr = long2str(long_num, buff + conversion_buff_size, format)))
				goto conversion_error;
...
void * __StringWrite(void * osc, const char * Buffer, size_t NumChars) {
	...
	CharsToBeWritten = ((Oscp->CharsWritten + NumChars) <= Oscp->MaxCharCount) ? NumChars : Oscp->MaxCharCount - Oscp->CharsWritten;
...
int printf(const char * format, ...)
{
...

	{
		...
		result = __pformatter(&__FileWrite, (void *)stdout, format, args);
	}
...
int vprintf(const char * format, va_list arg)
{
	...
	__begin_critical_region(files_access);
	retval = __pformatter(&__FileWrite, (void *)stdout, format, arg);
	__end_critical_region(files_access);
	return(retval);
}
...
int vfprintf(FILE * file, const char * format, va_list arg)
{
	...
	__begin_critical_region(files_access);
	retval = __pformatter(&__FileWrite, (void *)file, format, arg);
	__end_critical_region(files_access);
	return(retval);
}
